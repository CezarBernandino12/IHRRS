...
<!-- Sidebar Section -->
	...
	<section id="content">
	 <nav>
			...
			<span id="userGreeting" style="display:none;">Hello Nurse!</span>
		</nav>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch('../php/getUserName.php')
    ...
    .then(data => {
      ...
      if (sidebarName) {
        sidebarName.textContent = data.full_name || 'Nurse User';
      }
    })
    .catch(error => {
      console.error('Error fetching sidebar user name:', error);
      const sidebarName = document.getElementById('sidebarUserName');
      if (sidebarName) sidebarName.textContent = 'Nurse User';
    });
});
</script>
